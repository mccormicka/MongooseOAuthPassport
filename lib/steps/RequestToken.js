    ...
    function requestToken(req, res, parse, next) {
        ...
        passport.authenticate('consumer', {session: false})(req, res, function () {
            var token = oauth.requestToken(parse, function (consumer, callbackURL, params, done) {
                if (options.version === '1.0a' && !callbackURL) {
                    //1.0a requires a callbackURL.
                    return done(
                        {
                            status: 400,
                            code: 'parameter_absent&oauth_parameters_absent=callbackURL',
                            message: 'You must include a callbackURL in your request for a token'
                        }
                    );
                }

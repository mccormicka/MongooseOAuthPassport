        ...
        passport.authenticate('consumer', {session: false})(req, res, function () {
            oauth.accessToken(
                function (requestToken, verifier, info, done) {
                    if (verifier !== info.verifier || requestToken !== info.oauth.token || !info.valid) {
                        log.warn('Invalid verifier token combination', verifier, ' info ', info);
                        return done(null, false);
                    }
                    //Invalidate request token as it can only be used once.
                    info.valid = false;
                    info.save(function (err) {
                        if(err){
                            log.error('Unable to invalidate Request Token!', info);
                            return done(null, false);
                        }
                ...
                function (consumer, requestToken, info, done) {
                    ...
                    consumer[CREATE_ACCESS_TOKEN]({
                        ...
                        log.debug('Created Access Token', token.key, token.secret);
